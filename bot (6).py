...
anime_cache = {} # {url: {"episodes": {ep_num: {quality: {server: link}}}}}
...
def get_episodes(anime_url):
    ...
    try:
        ...
        # Scrape the download page with precise quality and server mapping
        # The structure is: <h2>Episode: XX</h2> followed by <p>Quality [Size]</p> followed by <p>Server Links</p>
        for element in soup2.find_all(['h2', 'h3', 'h4', 'p', 'div']):
            ...
            if current_ep:
                # Detect Quality (e.g., 480P, 720P, 1080P, 1080P HQ)
                quality_match = re.search(r'(\d+P(?:\s*HQ)?)', text, re.I)
                if quality_match:
                    quality = quality_match.group(1).upper()
                    # The links are usually in the next sibling or within the same element
                    ...
                    if links:
                        ...
                        episodes[current_ep][quality].update(links)
...
def get_direct_link(url):
    try:
        # Step 1: Handle links.animedubhindi.me redirection
        if "links.animedubhindi.me/re.php" in url or "redirect.php" in url:
            r = requests.get(url, timeout=10, allow_redirects=True)
            url = r.url
            
        # Step 2: Handle Filepress/Fpgo File Page
        if "fpgo.xyz/file/" in url or "filepress.wiki/file/" in url:
            r = requests.get(url, timeout=10)
            soup = BeautifulSoup(r.text, 'html.parser')
            direct_btn = soup.find('a', string=re.compile(r'Direct Download', re.I))
            if direct_btn:
                url = direct_btn['href']
            else:
                for a in soup.find_all('a', href=True):
                    if 'direct' in a.text.lower() or 'download' in a.text.lower():
                        url = a['href']
                        break

        # Step 3: Handle Filepress Download Page (with "Download Now" button)
        if "/download/" in url:
            r = requests.get(url, timeout=10)
            soup = BeautifulSoup(r.text, 'html.parser')
            final_btn = soup.find('a', string=re.compile(r'Download Now', re.I))
            if final_btn:
                return final_btn['href']
...
async def help_cmd(client, message):
    help_text = (
        ...
        "4. Choose the quality (480p to 1080p HQ).\n"
...
async def select_quality(client, callback_query):
    ...
    sorted_q = sorted(qualities.keys(), key=lambda x: int(re.search(r'\d+', x).group()) if re.search(r'\d+', x) else 0)
    buttons = [[InlineKeyboardButton(q, callback_data=f"dl_{anime_idx}_{ep_num}_{q}")] for q in sorted_q]
...
async def download_video(client, callback_query):
    ...
    raw_link = links.get('Fprs') or links.get('Hcloud') or links.get('Multi') or list(links.values())[0]
    
    msg = await callback_query.message.edit(f"⏳ **Bypassing Ads & Fetching Direct Link...**")
    
    dl_link = get_direct_link(raw_link)
        ...
        with open(file_path, 'wb') as f:
            ...
            for chunk in r.iter_content(chunk_size=1024*1024):
                if chunk:
                    ...
                    await progress_callback(downloaded, total_size, msg, start_time, "Downloading")
