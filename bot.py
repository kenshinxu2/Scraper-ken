...
def get_episodes(anime_url):
    ...
    try:
        ...
        # Enhanced quality fetching logic
        for element in soup2.find_all(['h2', 'h3', 'h4', 'p', 'div']):
            ...
            if current_ep:
                # Look for all qualities (480p, 720p, 1080p, 2160p)
...
def get_direct_link(url):
    # This function handles the Filepress (Fprs) bypass
    try:
        # First, handle the links.animedubhindi.me redirection if needed
        if "links.animedubhindi.me/re.php" in url or "redirect.php" in url:
            r = requests.get(url, timeout=10, allow_redirects=True)
            url = r.url
            
        # Now handle Filepress/Fpgo
        if "fpgo.xyz" in url or "filepress" in url:
            # We need to get the direct download link from the page
            # Usually, it's a button with "Direct Download"
            r = requests.get(url, timeout=10)
            soup = BeautifulSoup(r.text, 'html.parser')
            direct_btn = soup.find('a', string=re.compile(r'Direct Download', re.I))
            if direct_btn:
                return direct_btn['href']
            
            # If not found, try to find any link that looks like a download link
            for a in soup.find_all('a', href=True):
                if 'download' in a.text.lower() or 'direct' in a.text.lower():
                    return a['href']
        
        return url # Return original if no bypass found
    except Exception as e:
...
async def help_cmd(client, message):
    help_text = (
        ...
        "4. Choose the quality (480p to 1080p).\n"
